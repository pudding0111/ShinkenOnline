...
class FakeRandomBattleScreen extends StatefulWidget {
  @override
  _FakeRandomBattleScreenState createState() => _FakeRandomBattleScreenState();
}

class _FakeRandomBattleScreenState extends State<FakeRandomBattleScreen>  with SingleTickerProviderStateMixin {
  ...
  List<String> enemySkillNoList = ['No1', 'No2', 'No3', 'No4', 'No5', 'No6', 'No7', 'No8', 'No9', 'No10'];
...
//リアクション変数
  ...
  _loadEnemyInfo() {
    setState(() {
      ...
      List<String> rankList = ['bronze', 'silver', 'gold', 'platina','elite',];
      List<String> rankNumbers = ['1', '2', '3'];
      String rank = rankList[Random().nextInt(rankList.length)];
      String rankNumber = rankNumbers[Random().nextInt(rankNumbers.length)];
      if (rank.contains('bronze')) {
        enemyRankJapan = 'ブロンズ' + rankNumber;
      } else if (rank.contains('silver')) {
        enemyRankJapan = 'シルバー' + rankNumber;
      } else if (rank.contains('gold')) {
        enemyRankJapan = 'ゴールド' + rankNumber;
      } else if (rank.contains('platina')) {
        enemyRankJapan = 'プラチナ' + rankNumber;
      ...
      enemyWinStreak = [0, 1, 2][Random().nextInt(3)];
  ...
  _loadSkillList() async {
    ...
    setState(() {
      mySkillNoList = (prefs.getStringList('RandomSkillNoList') ?? ['No11', 'No14', 'No3', 'No42', 'No29', 'No26', 'No37', 'No34', 'No46', 'No6']);
  ...
  List<Map<String, dynamic>> scenes = [
    ...
    {'scene': 'declareSelect', 'seconds': 10},//10
  ...
  void skipCurrentScene() {
    ...
    int skipSceneInt = Random().nextInt(10);

    Future.delayed(Duration(milliseconds: 400 * skipSceneInt + 1500), () {
      mySkipState = false;
...
void showMyComment(String newComment) {
  ...
  Future.delayed(Duration(milliseconds: 500 * commentIndex + 1500), () {
    if (commentIndex < 7){
...
void showEnemyComment() {
  _completer = Completer<void>();
  if (_isEnemyCommented || !enemyBattleState) return;
  setState(() {
    int commentIndex = Random().nextInt(6);
    switch (commentIndex) {
      case 0:
      enemyComment = angryComments[Random().nextInt(angryComments.length)];
      break;

      case 1:
      enemyComment = happyComments[Random().nextInt(happyComments.length)];
      break;

      case 2:
      enemyComment = coolComments[Random().nextInt(coolComments.length)];
      break;

      case 3:
      enemyComment = thinkComments[Random().nextInt(thinkComments.length)];
      break;

      case 4:
      enemyComment = chickenComments[Random().nextInt(chickenComments.length)];
      break;

      case 5:
      enemyComment = hakuryokuComments[Random().nextInt(hakuryokuComments.length)];
      break;
...
@override
          ...
          Positioned( //相手の宣言ポイント
            ...
            child: Column(
              ...
              children: [
                ...
                if (nowScene == 'battleSelect' || nowScene == 'skillSelect' || nowScene == 'declareSelect')
                 Padding(
  ...
  void performMySkillEffect(String skillNo, String otherSkillNo, String battleHand, String declareHand, String otherBattleHand, String otherDeclareHand, int point, int otherPoint, int changeSkill, int changeOtherSkill, int otherChangeSkill, int otherChangeOtherSkill, int declarePoint, int otherDeclarePoint, String skillSelect, String otherSkillSelect, List<Map<String, String>> cards, List<Map<String, String>> otherCards) {
    ...
    print((combinedList.sublist(0,mid)).length);
    for (int i = 0; i < (combinedList.sublist(0,mid)).length; i++){
      print(i);
      combinedList[i]['belong'] = 'mine';
    }
    for (int i = 0; i < (combinedList.sublist(mid)).length; i++){
      print('${i + mid}');
      combinedList[i + mid]['belong'] = 'enemy';
    }
    print(combinedList);
    setState(() {
      myCards = combinedList.sublist(0, mid);
      enemyCards = combinedList.sublist(mid);
    });
  ...
  void performEnemySkillEffect(String skillNo, String otherSkillNo, String battleHand, String declareHand, String otherBattleHand, String otherDeclareHand, int point, int otherPoint, int changeSkill, int changeOtherSkill, int otherChangeSkill, int otherChangeOtherSkill, int declarePoint, int otherDeclarePoint, String skillSelect, String otherSkillSelect, List<Map<String, String>> cards, List<Map<String, String>> otherCards) {
    ...
    for (int i = 0; i < (combinedList.sublist(0,mid)).length; i++){
      print(i);
      combinedList[i]['belong'] = 'enemy';
    }
    for (int i = 0; i < (combinedList.sublist(mid)).length; i++){
      print('${i + mid}');
      combinedList[i + mid]['belong'] = 'mine';
    }
    print(combinedList);
    setState(() {
      enemyCards = combinedList.sublist(0, mid);
      myCards = combinedList.sublist(mid);
    });
...
/* fakeStrategyBattleからの変更点

ただ、loadSkillのところをprefs.get('RandomSkillNoList')に変えただけ
