...
class MakeIconScreen extends StatefulWidget {
  @override
  _MakeIconScreenState createState() => _MakeIconScreenState();
}

class _MakeIconScreenState extends State<MakeIconScreen>  with TickerProviderStateMixin{
  ...
  void _unfocusAll() {
    ...
    SystemChannels.textInput.invokeMethod('TextInput.hide'); // キーボード非表示
  }
    ...
    return Scaffold(
      ...
      Stack(
        ...
        child: PageView.builder(
          ...
          physics: BouncingScrollPhysics(), // オーバースクロールを有効にする
                  ...
                  SingleChildScrollView(
                    ...
                    Column(
                      children: [
                        ...
                        if (originId == uid)
                        CustomImageButton(screenWidth: screenWidth, buttonText: 'リセット', onPressed: () {showResetAlert();}),
                        Row(
  ...
  */
  ...
  Future<void> fetchLatestIconsWithLikes({int limit = 16}) async {
  ...
  Future<void> fetchOlderIconsWithLikes({int limit = 16}) async {
    if (isLoading || isEndOfPosts) return;  // Prevent multiple simultaneous fetches
    setState(() {
      isLoading = true;
    });
...
class __buildPixelGridFromFlatState extends State<_buildPixelGridFromFlat> {
  @override
  Widget build(BuildContext context) {
    final cellSize = ((widget.postHeight * 0.8) / widget.gridNumber).floorToDouble();
